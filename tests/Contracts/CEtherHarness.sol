...
contract CEtherHarness is CEther {
    ...
    function harnessDoTransferIn(address from, uint256 amount)
        public
        payable
        returns (uint256)
    {
    ...
    function harnessDoTransferOut(address payable to, uint256 amount)
        public
        payable
    {
    ...
    function harnessRequireNoError(uint256 error, string calldata message)
        external
        pure
    {
...
contract CEtherScenario is CEther {
    ...
    function getBlockNumber() internal view returns (uint256) {
        ComptrollerScenario comptrollerScenario =
            ComptrollerScenario(address(comptroller));
